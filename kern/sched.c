...
//			cprintf("kern_sched.env_run: envs[%d]\n", i);
 ...
 // if (curenv == NULL) curenv = &envs[0];
 //int cur_envx = ENVX(curenv->env_id); 
 //// BUG: 无法进入循环
 ////for (int i = start_envx; i != start_envx; i = ENVX(i+1)) 
 // for (int i = ENVX(cur_envx+1); i != cur_envx; i = ENVX(i+1)) 
 //  if (envs[i].env_status == ENV_RUNNABLE)
 //   env_run(&envs[i]);
