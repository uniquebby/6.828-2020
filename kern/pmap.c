...
mem_init(void)
{
  ...
  bytes_to_alloc = sizeof(struct Env) * NENV;
  ...
  memset(envs, 0, ROUNDUP(bytes_to_alloc, PGSIZE));
	...
	// Your code goes here:
  // bug: 映射pages本身的地址，而不是他指代的物理页的地址
  //boot_map_region(kern_pgdir, UPAGES, ROUNDUP(sizeof(struct PageInfo) * npages, PGSIZE), page2pa(pages), (PTE_W|PTE_P));  
  //BUG: 权限错误，UPAGES处地址应该是只读，
  //而npages本来的地址（也就是内核以上的地址）才是内核读写
  ...
  boot_map_region(kern_pgdir, UPAGES, ROUNDUP(sizeof(struct PageInfo) * npages, PGSIZE), PADDR(pages), (PTE_U|PTE_P));  
	...
	// LAB 3: Your code here.
  //boot_map_region(kern_pgdir, UENVS, ROUNDUP(sizeof(struct Env) * NENV, PGSIZE), PADDR(envs), (PTE_W|PTE_P));  
  ...
  //boot_map_region(kern_pgdir, KSTACKTOP-PTSIZE, ROUNDUP(PTSIZE-KSTKSIZE, PGSIZE), npages * (PGSIZE+1)), (PTE_W|PTE_P); 
...
mem_init_mp(void)
{
  ...
  // BUG:cup0的stack之前用的是bootstacak,所以也要重新map
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
  ...
  if (!(cur_pde & PTE_P)) {
    ...
    //bug记录：没有把物理地址转换为虚拟地址
    res = (pte_t*)page2kva(page);
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
  ...
  for (size_t i = 0; i < size; i += PGSIZE, pa += PGSIZE, va += PGSIZE) {
    ...
    *pte_ptr = pa | (perm|PTE_P);
    //cprintf("boot_map_region: map %x to %x\n", va, pa);
  }  
}
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
  ...
  struct PageInfo *page_info = pa2page(PTE_ADDR(*pte_ptr)); 
	return page_info;
}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
  ...
  if (va+len >= (void*)ULIM) {
    ...
    //bug:void *end = (void*)ROUNDDOWN(va + len, PGSIZE);
    void *end = (void*)ROUNDUP(va + len, PGSIZE);

    for (; start < end; start += PGSIZE) {
      //BUG:调了两个多小时
      //pte_t *pte_ptr = pgdir_walk(env->env_pgdir, va, false);
      pte_t *pte_ptr = pgdir_walk(env->env_pgdir, start, false);
      if (pte_ptr == NULL || ((*pte_ptr & perm) != perm)) {
        //BUG: 逻辑反了
        //user_mem_check_addr = (uintptr_t)(start < va ? start : va);
