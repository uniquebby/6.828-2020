...
load_icode(struct Env *e, uint8_t *binary)
{
  ...
  //bug:不能直接用p_va复制，因为现在是内核的地址空间，不是
  //要加载的这个进程的
  //所以加了下面这行,这也说明了区分两个进程的最重要点是
  //的eip等上下文不同
  lcr3(PADDR(e->env_pgdir));
  for (; ph < eph; ph++) {
    if (ph->p_type == ELF_PROG_LOAD) {
      region_alloc(e, (void*)ph->p_va, ph->p_memsz);  
//      cprintf("load_icode: region_alloc start_va = %x and size = %d\n", ph->p_va, ph->p_memsz);
